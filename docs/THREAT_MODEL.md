...
| Enrollment-QR wird kopiert | QR gilt wie der Authentik-Enrollment-Token als Geheimnis, wird nur fünf Minuten und modus-/principal-spezifisch im Browser angezeigt und nach erfolgreichem Einlösen serverseitig entwertet. Beim persönlichen Gerät besteht die direkte Benutzerbindung bereits vor der QR-Ausgabe |
...
| gestohlenes Kennwort einer berechtigten Leitung öffnet die Geräte-Einrichtung | der anwendungsbezogene Authentik-Authorization-Flow verlangt zusätzlich ein bereits eingerichtetes TOTP oder WebAuthn und verweigert Benutzer ohne starken Faktor; die Seite bietet keine MFA-Einrichtung an |
