...
| vorheriger Benutzer bleibt auf Shared Tablet angemeldet | kein `offline_access`, kein persistenter AuthState; explizite Abmeldung und Ausschalten des Bildschirms verwerfen Token, Push-Zuordnung und sämtliche WebView-Daten. Minimieren bleibt für extern geöffnete Links erlaubt; der Benutzerhinweis benennt deshalb das Ausschalten des Bildschirms als Sitzungsende |
...
| Webseite greift unbemerkt auf Kamera/Mikrofon zu | der WebView verweigert jede Medienfreigabe unabhängig von Domain; `CAMERA` und `RECORD_AUDIO` sind nicht im Manifest |
...
| fremder QR-Code schleust eine URL oder einen Token ein | die normale Kamera beziehungsweise der Browser darf Links öffnen, aber die App akzeptiert ausschließlich den exakten App-Deep-Link oder `https://geraete.mission-leben.de/install` mit Geheimnis im Fragment und genau einem plausiblen Token; die serverseitige Einlösung prüft zusätzlich den kurzlebigen Enrollment-Token |
...
| gestohlenes Kennwort öffnet die Geräte-Einrichtung | der anwendungsbezogene Authentik-Authorization-Flow verlangt zusätzlich ein bereits eingerichtetes TOTP, einen Passkey oder die Bestätigung auf einem schon registrierten persönlichen Gerät und verweigert Benutzer ohne kompatiblen Faktor; die Seite bietet keine MFA-Einrichtung an |
