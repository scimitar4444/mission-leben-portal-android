...
Der reguläre Einrichtungsweg ist der separate Container in `enrollment-portal/`. Aktive Mitarbeiter mit vorhandenem TOTP, Passkey oder registrierter App-Bestätigung dürfen dort ausschließlich ihr eigenes persönliches Gerät registrieren; für das erste Gerät bleibt TOTP oder Passkey erforderlich. IT, Leitungen in der Zentrale, EL und PDL erhalten zusätzlich die Verwaltungsansicht. Der Container grenzt EL, PDL und zentrale Leitungen auf ihre vorhandenen `ORG_*`-Gruppen ein; nur IT besitzt globalen Suchzugriff. Für persönliche Geräte wird die direkte Benutzerbindung vor Ausgabe des Einmal-Links oder QR-Codes angelegt, für Shared Tablets die Bindung an genau eine Einrichtung.
