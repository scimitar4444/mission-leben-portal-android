...
Für wiederholbare Ende-zu-Ende-Tests kann zusätzlich die parallel installierbare Debug-App `de.missionleben.portal.debug` verwendet werden. Sie besitzt eine eigene Android-Keystore- und Endpoint-Identität und verwendet ausschließlich den zweiten, exakt eingetragenen Redirect `de.missionleben.portal.debug:/oauth2redirect`. `authentik/bootstrap_e2e_debug.py` kann nach gesonderter Freigabe das reservierte interne Testkonto `ml-portal-e2e` und ein bestätigtes Test-TOTP vorbereiten. Die isolierte Organisation `ORG_E2E_TEST` und die kanonische Rolle `BR_EINRICHTUNGSLEITUNG` müssen bereits vorhanden sein; das Skript erstellt keine Gruppen und vergibt keine Einrichterrolle. Bestehende Altmitgliedschaften werden nicht migriert. Eine Testberechtigung entscheidet ausschließlich die Gruppenverwaltung. `ORG_E2E_TEST` ist keine produktiv auswählbare Einrichtung; das Testkonto erhält dadurch keine persönliche Mitarbeiterzulassung. Passwort und TOTP-Schlüssel kommen aus root-only Dateien und werden weder im Repository noch in der Skriptausgabe gespeichert. Das Konto erhält keine Anwendungsgruppen und damit keinen Zugriff auf echte Zimbra-, Nextcloud- oder Talk-Daten.

Für die getrennte Pflege und geprüfte Übertragung der Portal-/E2E-Bootstraps gilt [BOOTSTRAP_MAINTENANCE.md](BOOTSTRAP_MAINTENANCE.md). Eine Dateiaktualisierung führt keinen Bootstrap aus. Beide Skripte benötigen zur späteren Ausführung eine gesonderte Freigabe, `ML_AUTHENTIK_BOOTSTRAP_APPLY=1` und eine ausdrücklich geprüfte Authentik-Version.
...
Der reguläre Einrichtungsweg ist der separate Container in `enrollment-portal/`. Aktive Mitarbeiter mit vorhandenem TOTP, Passkey oder registrierter App-Bestätigung dürfen dort ausschließlich ihr eigenes persönliches Gerät registrieren; für das erste Gerät bleibt TOTP oder Passkey erforderlich. IT, EL und PDL erhalten zusätzlich die Verwaltungsansicht. Geschäftsbereichs-, Geschäftseinheits-, Abteilungs- und Geschäftsführungsleitungen sind keine Einrichter. Der Container zeigt IT alle kanonisch als echte Einrichtung klassifizierten Standorte und grenzt EL sowie PDL auf sämtliche eigenen echten Einrichtungen ein. Eine Mehrfachzuordnung des Einrichters bleibt vollständig auswählbar; jedes Shared Tablet wird dennoch an genau eine Einrichtung gebunden. Untergruppen können über Authentiks effektive `all_groups()`-Mitgliedschaft Benutzerzugriff vermitteln, sind aber selbst nie Gerätestandorte. Für persönliche Geräte wird die direkte Benutzerbindung vor Ausgabe des Einmal-Links oder QR-Codes angelegt.
...
Gerät sperren: Unter **Endpoint Devices → Devices** in den Attributen `mission-leben.de/status=disabled` setzen und Zeitpunkt sowie Grund dokumentieren. Die Android-Policies lehnen dieses Device anschließend serverseitig ab. Der automatische Abgleich sendet gezielt und einmalig pro dokumentiertem Sperrstand nur an dieses Gerät `refresh_security_state`, widerruft anschließend dessen ntfy-Identitäten und entfernt ausschließlich dessen Kommunikationsdaten. Die App prüft Authentik selbst und löscht Sitzung und Webdaten. Ein sofortiger manueller Lauf ist mit `systemctl start mission-leben-device-offboarding.service` möglich. Löschen oder ein sofortiges Ablaufdatum sind für die normale Sperrung nicht zulässig, weil die Nachverfolgbarkeit erhalten bleiben muss.

Mitarbeiter-Offboarding: Das Mitarbeiterkonto wird in Authentik deaktiviert. `authentik/reconcile_inactive_personal_devices.py` wird durch `mission-leben-device-offboarding.timer` alle fünf Minuten ausgeführt. Es verarbeitet ausschließlich direkte Benutzerbindungen an Device Access Groups mit `mission-leben.de/purpose=android-portal` und `mission-leben.de/mode=personal`, synchronisiert die zugeordneten Geräte als sichtbare, nicht autorisierende Übersicht im Benutzerattribut `mission-leben.de/devices`, deaktiviert bei inaktiven Konten deren Geräte, entzieht Authentik-Sitzungen sowie Core-/OAuth-Tokens und übergibt das stabile Subject an die getrennte Bridge-Bereinigung. Das Authentik-`DuoDevice` für die App-Bestätigung bleibt nur bestehen, solange der aktive Benutzer mindestens ein aktives persönliches Endpoint Device besitzt; nach Deaktivierung oder dem manuellen Löschen des letzten Endpoint Device wird es beim nächsten Abgleich entfernt. Die Bridge sendet vor dem Löschen ihrer ntfy-Zuordnung den inhaltslosen Weckimpuls `refresh_security_state` und widerruft Reader und Writer. Die App prüft den Endpoint daraufhin selbst bei Authentik, löscht bei Sperre ihre Sitzung und Webdaten und schließt einen offenen internen Browser. Bei fehlender Netzverbindung geschieht dieselbe Prüfung bei Rückkehr in die App und unmittelbar vor dem Öffnen einer geschützten Web-App. Der Nextcloud-OIDC-Provider verwendet zusätzlich `user_oidc`-Backchannel-Logout. Shared-Geräte werden nicht pauschal gesperrt, wenn ein einzelnes Mitglied der Einrichtungsgruppe ausscheidet. Zimbra verwendet derzeit SAML; das Beenden einer bereits bestehenden Zimbra-Mailboxsitzung benötigt noch einen eigenen, sicher authentifizierten Zimbra-Administrationsadapter.

## 6. App-Bestätigung als Authentik-Faktor

Die App-Bestätigung verwendet die in Authentik Community vorhandene Duo-Stufe, aber keinen Duo-Cloud-Dienst und keine Enterprise-Funktion. Die Stufe spricht die signierte Duo Auth API gegen den separaten Kommunikationscontainer. Authentik bleibt für Benutzer, `DuoDevice`, MFA-Auswahl und Endpoint Devices zuständig; die Bridge speichert nur kurzlebige Anmeldeanfragen und die bereits für Kommunikation benötigte Gerätezuordnung. Im Geräteportal ist sie für Benutzer mit einem bereits registrierten persönlichen Gerät eine zusätzliche Option neben TOTP und Passkey. Für die erste Registrierung bleiben TOTP und Passkey erforderlich.
...
Die geöffnete App sieht eine Anfrage durch ihr Vordergrund-Polling spätestens nach etwa zwei Sekunden; ntfy weckt zusätzlich die geschlossene App mit ausschließlich der zufälligen Anfrage-ID. Der Dialog zählt anhand des serverseitigen Ablaufzeitpunkts sekundengenau herunter; bei Ablauf verschwinden die Entscheidungsschaltflächen. Ist die App bereits entsperrt, erscheint keine zweite Biometrieabfrage. Anwendungsname oder andere Anmeldedaten werden nicht über ntfy versendet.
