...
`GET /api/v1/devices/status` nimmt ausschließlich einen Authentik-Device-Token im Schema `Bearer+Agent` an. Der Endpunkt validiert den Token über den Authentik-Agent-Connector, liest anschließend den zugehörigen Endpoint direkt aus Authentik und verweigert deaktivierte oder abgelaufene Geräte. App und Kommunikations-Bridge verwenden diese Live-Prüfung; es entsteht keine parallele Freigabedatenbank.
