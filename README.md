...
- Der Webcontainer gibt grundsätzlich keiner Webseite Kamera oder Mikrofon; die App fordert selbst keine Kameraberechtigung an. Talk läuft deshalb bewusst als reiner Chat. Die App blendet die Nextcloud-Kopfzeile aus und öffnet beim nächsten Start direkt den zuletzt verwendeten Raum; Abmelden/Zurücksetzen löscht diese lokale Erinnerung.
- Cookies, Webspeicher, HTTP-Zugangsdaten, Cache und geschützte Downloads werden bei sicherer Abmeldung oder Profilwechsel gelöscht; auf Shared Tablets zusätzlich beim Ausschalten des Bildschirms
...
- vorbereitete Enrollment-QR-Codes werden mit der normalen Kamera-App gescannt; der verifizierte HTTPS-Link führt zur Installation oder direkt zurück in die App, ohne dass die App Kamerazugriff benötigt
...
- persönliche Selbstregistrierung für aktive Mitarbeiter mit vorhandenem TOTP, Passkey oder bereits registrierter App-Bestätigung: Anmeldung im geschützten App-Browser, direkte Bindung an das eigene Authentik-Konto und automatischer Rücksprung zur App; für das allererste Gerät bleibt TOTP oder Passkey erforderlich, die Seite richtet keinen Faktor ein
...
Die App enthält keinen eigenen QR-Scanner und benötigt keine Kameraberechtigung. Die normale Kamera-App öffnet den vom Geräte-Einrichtungscontainer erzeugten verifizierten HTTPS-Link. Ist Mission Leben Zentral bereits installiert, übernimmt Android den Link direkt; andernfalls führt dieselbe Seite erst durch Installation und dann über einen expliziten Knopf zurück in die App. Die App akzeptiert ausschließlich den vollständigen Enrollment-Link mit Token-ID und Gerätemodus. Ein alter Token-only-Link funktioniert nur noch auf einem Gerät, dessen Modus bereits lokal feststeht. Beliebige URLs und rohe Texte werden verworfen.
